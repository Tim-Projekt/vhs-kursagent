...
export function Weather({
  ...
  return (
    <div className="relative flex w-full flex-col gap-3 overflow-hidden rounded-lg border border-border/60 bg-card p-4">
      <div>
        <div className="mb-2 flex items-center justify-between">
          ...
          <div className="text-muted-foreground/70 text-xs">
            {format(new Date(weatherAtLocation.current.time), "d. MMM, HH:mm")}
          </div>
        </div>
        ...
        <div className="mb-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="text-primary">
              {isDay ? <SunIcon size={30} /> : <MoonIcon size={30} />}
            </div>
        ...
        <div className="rounded-md border border-border/50 bg-muted/60 p-3">
          ...
          <div className="flex justify-between gap-1">
            {displayTimes.map((time, index) => {
              ...
              return (
                <div
                  className={cx(
                    "flex min-w-0 flex-1 flex-col items-center gap-1 rounded-sm px-1 py-1.5",
                    {
                      "bg-accent": isCurrentHour,
                    }
                  )}
                  key={time}
                >
                  <div className="font-medium text-muted-foreground/80 text-xs">
                    {index === 0 ? "Jetzt" : format(hourTime, "HH:mm")}
                  </div>
        ...
        <div className="mt-2 flex justify-between text-muted-foreground/70 text-xs">
          <div>
            Sonnenaufgang:{" "}
            {format(new Date(weatherAtLocation.daily.sunrise[0]), "HH:mm")}
          </div>
          <div>
            Sonnenuntergang:{" "}
            {format(new Date(weatherAtLocation.daily.sunset[0]), "HH:mm")}
          </div>
        </div>
      </div>
    </div>
